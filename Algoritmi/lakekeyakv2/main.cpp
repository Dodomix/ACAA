/* This is a main function that is used for Keyak algorithm testing
 *
 * Encryption input:
 * - Plaintext P
 * - Associated data AD
 ...
 * - Authentication tag T (len(T) == len(K)) concatenated with C
 * ------------------------------------------------------------------
 * Decryption input:
 * - Ciphertext C
...
//----------CRYPTOGRAPHIC PARAMETERS-------------------------------------------------
    ...
    if((buffer = (unsigned char*)malloc(file_size)) == NULL ){
        ...
        buffer = (unsigned char*)memset(buffer,'\0', file_size);
    }
    ...
    if( (ciphertext = (unsigned char *)malloc(ciphertext_tag_size)) == NULL){
        ...
        ciphertext = (unsigned char *)memset(ciphertext, '\0', ciphertext_tag_size);
    ...
    // Encryption
    status = crypto_aead_encrypt(ciphertext, ciphertext_len, buffer, file_size, AD, 6, 0, npub, k);
    if(status != 0){
    ...
    status = crypto_aead_decrypt(plaintext, &plaintext_len, nsec, ciphertext, *ciphertext_len, AD, 6, npub, k);
