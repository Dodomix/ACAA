...
void inline morus256_stateupdate(const uint64_t* msgblk, uint64_t state[][4]) 
{
	uint64_t temp;
...
void morus256_initialization(const uint8_t *key, const uint8_t *iv, uint64_t state[][4])
{
	int i;
	...
	for (i = 0; i < 16; i++) morus256_stateupdate(temp, state);
	for (i = 0; i < 4;  i++) state[1][i] ^= ((uint64_t*)key)[i];
}

//the finalization state of morus256
void morus256_tag_generation(uint64_t msglen, uint64_t adlen, uint8_t *c, uint64_t state[][4])
{
	int i;
	uint8_t t[32];
	...
	for (i = 0; i < 10; i++) morus256_stateupdate((uint64_t*)t, state);
...
int morus256_tag_verification(uint64_t msglen, uint64_t adlen, const uint8_t *c, uint64_t state[][4])
{
	int i;
	uint8_t t[32];
	int check = 0;
...
inline void morus256_enc_aut_step(const uint8_t *plaintextblock,
									  uint8_t *ciphertextblock, uint64_t state[5][4])
{
	...
	morus256_stateupdate(((uint64_t*)plaintextblock), state);

}


// one step of decryption: it decrypts a 32-byte block
inline void morus256_dec_aut_step(uint8_t *plaintextblock,
...
void morus256_enc_aut_partialblock(const uint8_t *plaintext,
...
void morus256_dec_aut_partialblock(uint8_t *plaintext,
	...
	)
{
	...
	uint64_t morus256_state[5][4];

	//initialization
	morus256_initialization(k, npub, morus256_state);

	//process the associated data
	for (i = 0; (i+32) <= adlen; i += 32) {
		morus256_enc_aut_step(ad+i, ciphertextblock, morus256_state);
	}

	//deal with the partial block of associated data
	//in this program, we assume that the message length is a multiple of bytes.
	if (  (adlen & 0x1f) != 0 )  {
		morus256_enc_aut_partialblock(ad+i, ciphertextblock, adlen & 0x1f, morus256_state);
	}

	//encrypt the plaintext
	for (i = 0; (i+32) <= mlen; i += 32) {
		morus256_enc_aut_step(m+i, c+i, morus256_state);
	}

	// Deal with the partial block
	// In this program, we assume that the message length is a multiple of bytes.
	if (  (mlen & 0x1f) != 0 )  {
		morus256_enc_aut_partialblock(m+i, c+i, mlen & 0x1f, morus256_state);
	}

	//finalization stage, we assume that the tag length is a multiple of bytes
	morus256_tag_generation(mlen, adlen, c, morus256_state);
	*clen = mlen + 16;
	return 0;
}
	...
	)
{
	...
	*mlen = clen - 16;
	for (i = 0; (i+32) <= *mlen; i += 32) {
		morus256_dec_aut_step(m+i, c+i, morus256_state);
	}

	// Deal with the partial block
	// In this program, we assume that the message length is a multiple of bytes.
	if (  (*mlen & 0x1f) != 0 )  {
		morus256_dec_aut_partialblock(m+i, c+i, *mlen & 0x1f, morus256_state);
	}

	// we assume that the tag length is a multiple of bytes
	// verification
	return morus256_tag_verification(*mlen, adlen, c, morus256_state);
}
